...
class TestAccuracyModeSamplingGuards:
    ...
    def test_preformat_skipped_when_cache_bust_enabled(
        self, initialized_dataset_manager
    ):
        """Cache-bust is incompatible with PAYLOAD_BYTES fast path.

        The worker's PAYLOAD_BYTES early-return bypasses the cache-bust dispatch
        in `_process_credit_with_session`, so any cache_bust target other than
        NONE must keep the dataset on the structured-turns path (no
        raw_payload preformatting).
        """
        from aiperf.common.enums import CacheBustTarget

        initialized_dataset_manager.user_config.input.prompt.cache_bust.target = (
...
class TestSelectMmapFormat:
    ...
    def test_select_format_conversation_when_no_raw_payload(
        self, initialized_dataset_manager
    ):
        """No raw_payload anywhere -> CONVERSATION format."""
    ...
    def test_select_format_payload_bytes_when_all_raw_payload(
        self, initialized_dataset_manager
    ):
        """All turns carry raw_payload -> PAYLOAD_BYTES format."""
        from aiperf.common.enums import MemoryMapFormat
        ...
        conversations = [
            ...
            Conversation(
                session_id="s2",
                turns=[Turn(role="user", raw_payload={"b": 2})],
            ),
        ]
        assert (
            initialized_dataset_manager._select_mmap_format(conversations)
    ...
    def test_select_format_rejects_mixed_raw_payload(self, initialized_dataset_manager):
        """Mixed raw_payload state -> ValueError mentioning uniformity."""
        conversations = [
            Conversation(
                session_id="s1",
                turns=[Turn(role="user", raw_payload={"a": 1})],
    ...
    def test_select_format_rejects_payload_bytes_when_cache_bust_enabled(
        ...
        """Cache-bust + raw_payload-producing loader must raise at format-selection.

        Loaders that natively populate ``Turn.raw_payload`` (raw_payload,
        inputs_json, mooncake_trace with ``payload`` field) would otherwise
        select PAYLOAD_BYTES and silently bypass cache-bust marker injection
        on the worker hot path. The format-selection guard fires before the
        backing store is built, so the operator sees a clear error early.
        """
        from aiperf.common.enums import CacheBustTarget
    ...
    def test_select_format_allows_conversation_when_cache_bust_enabled(
        self, initialized_dataset_manager
    ):
        """Cache-bust with no raw_payload (structured turns) -> CONVERSATION."""
