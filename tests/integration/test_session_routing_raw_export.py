...
- ``identity_headers`` (``session=X-Affinity,X-SMG-Routing-Key`` +
  ``root=X-Tree-ID``): every configured header equals ``x_correlation_id``
...
def _verify_session_id_header(by_session: dict[str, list[RawRecordInfo]]) -> None:
    for xcorr, recs in by_session.items():
        for rec in recs:
            headers = rec.request_headers or {}
            assert headers.get("X-Session-ID") == xcorr, (
                f"session {xcorr} turn {rec.metadata.turn_index}: headers={headers}"
            )
            assert "nvext" not in _payload_dict(rec), (
                f"session {xcorr}: header-mode must not mutate the body"
...
def _verify_identity_headers(by_session: dict[str, list[RawRecordInfo]]) -> None:
    """Two session-tier names plus a root-tier name; flat sessions are their
    own tree roots, so every header carries the session's X-Correlation-ID."""
    for xcorr, recs in by_session.items():
        for rec in recs:
            headers = rec.request_headers or {}
            for name in ("X-Affinity", "X-SMG-Routing-Key", "X-Tree-ID"):
...
_VERIFIERS: dict[str, Callable[[dict[str, list[RawRecordInfo]]], None]] = {
    ...
    "identity_headers": _verify_identity_headers,
}
...
@pytest.mark.parametrize(
    "mode, opts, verifier",
    [
        param("dynamo_headers", (), "dynamo_headers", id="dynamo_headers"),
        param("dynamo_nvext", (f"timeout_seconds={_TIMEOUT_SECONDS}",), "dynamo_nvext", id="dynamo_nvext"),
        param("smg_routing_key", (), "smg_routing_key", id="smg_routing_key"),
        param("session_id_header", (), "session_id_header", id="session_id_header"),
        param(
            "identity_headers",
            ("session=X-Affinity,X-SMG-Routing-Key", "root=X-Tree-ID"),
            "identity_headers",
            id="identity_headers",
        ),
    ],
...
async def test_session_routing_mode_reaches_wire(
    ...
    _VERIFIERS[verifier](by_session)
